...
export class EmployeeService {
  ...
  protected convertDateFromClient(employee: IEmployee): IEmployee {
    const copy: IEmployee = Object.assign({}, employee, {
      dob: employee.dob && employee.dob.isValid() ? employee.dob.format(DATE_FORMAT) : undefined,
      addDate: employee.addDate && employee.addDate.isValid() ? employee.addDate.format(DATE_FORMAT) : undefined,
      updateDate: employee.updateDate && employee.updateDate.isValid() ? employee.updateDate.format(DATE_FORMAT) : undefined,
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((employee: IEmployee) => {
        employee.dob = employee.dob ? moment(employee.dob) : undefined;
        employee.addDate = employee.addDate ? moment(employee.addDate) : undefined;
        employee.updateDate = employee.updateDate ? moment(employee.updateDate) : undefined;
      });
    }
    return res;
  }
