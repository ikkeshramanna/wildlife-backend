...
export class CountryService {
  ...
  protected convertDateFromClient(country: ICountry): ICountry {
    const copy: ICountry = Object.assign({}, country, {
      addDate: country.addDate && country.addDate.isValid() ? country.addDate.format(DATE_FORMAT) : undefined,
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((country: ICountry) => {
        country.addDate = country.addDate ? moment(country.addDate) : undefined;
      });
    }
    return res;
  }
